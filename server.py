...
def handle_data():
    ...
    while True:
            ...
            try:
                ...
                if ',' in data:
                    ...
                    if len(parts) == 5:
                        ...
                        name = parts[-1] # останній елемент це ім'я
                        players[conn] = {'id': pid, 'x': x, 'y': y, 'r': r, 'name': name} # зберігання даних про гравця
        ...
        for conn in list(players.keys()):
            ...
            try:
                packet = '|'.join([f"{p['id']},{p['x']},{p['y']},{p['r']},{p['name']}" #Підготовка пакета на відправку
...
while True:
    try:
        ...
        id_counter += 1
        players[conn] = {'id': id_counter, 'x': 0, 'y': 0, 'r': 20, 'name': None} # Додали ім'я None (потім клієнт його відправить)
