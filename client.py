    ...
    while running:
        try:
            ...
            if data == "LOSE":
                ...
                all_players = [list(map(int, p.split(',')[:4]))+ [p.split(',')[-1]] for p  in parts if len(p.split(',')) == 5] # додали отримання імені
                #+ [p.split(',')[-1]] от це ім'я додається до кожного гравця
        except:
            pass
...
while running:
    ...
    if not lose:
        ...
        try:
            msg = f"{my_id},{my_player[0]},{my_player[1]},{my_player[2]},{name}" # відправка свого пакета інфи
